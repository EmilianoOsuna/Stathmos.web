...
serve(async (req) => {
  ...
  try {
    ...
    // Accept monto == 0; only reject when a field is null or undefined
    if (proyecto_id == null || monto == null || metodo_cobro == null) {
      return new Response(JSON.stringify({ success: false, error: "Missing required fields: proyecto_id, monto, metodo_cobro", received: body }), { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 400 });
    }
    ...
    return new Response(JSON.stringify({ success: false, error: String(error?.message || error), stack: error?.stack || null }), { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 400 });
  }
});