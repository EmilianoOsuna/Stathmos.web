  ...
  const handleSubmit = async () => {
    ...
    try {
      for (const item of cart) {
        ...
        if (cantidadNum > Number(item.stock || 0)) {
  ...
  return (
    <div className={`flex-1 p-4 md:p-6 min-h-full page-enter ${darkMode ? "bg-[#16161e]" : "bg-gray-50"}`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h2 className={`text-lg font-semibold ${t}`}>Venta de Refacciones</h2>
          <p className={`text-xs ${st} mt-0.5`}>Selecciona refacciones y confirma la venta</p>
        </div>
      </div>
              ...
              {filtered.map((r) => (
                <div key={r.id} className={`py-3 flex items-center justify-between gap-3 ${rowH}`}>
                  ...
                  <div className="flex items-center gap-3">
                    <p className={`text-sm ${st}`}>${fmtMoney(r.precio_venta)}</p>
                    <button
                      onClick={() => addToCart(r)}
